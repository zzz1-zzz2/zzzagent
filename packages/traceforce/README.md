...
TUI 显示 assistant streaming、workspace/session 状态、工具卡片和权限弹窗。工具卡片支持展开、折叠、复制和关闭；关闭只影响可见 UI，不取消底层工具执行。
...
测试使用 FakeLLM、Fake SDK、本地假 MCP server 和临时 workspace，不需要网络或真实 API key。当前离线测试数量为 45 项（以实际运行结果为准）。
